...
public class ProcessData implements ProcessDataInterface{
    ...
    public ArrayList<String> displayedMailbox = new ArrayList<String>();
    ...
    @Override
    public int shouldReplyOK(int idx){
        try{
      
            ArrayList<Message> mailbox = this.mailbox;

            if(mailbox.get(idx).getType().compareTo("ELECTION") == 0) {
    ...
    @Override
    public void addToMailbox(String messageType, LocalTime time , int id) {
    ...
    @Override
    public boolean searchMailbox(String messageType, int timeInterval){
       try{
